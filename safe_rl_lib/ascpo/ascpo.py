    ...
    def compute_loss_vc(data):
        ...
        if 'delta' in exp_name and 'sub' in exp_name:
            ...
            if subsampling:
                ...
                return ((pred-cost_ret_downsample)**2).mean() + eta*(delta**2).mean()
            else:
                pred = ac.vc(obs)
                delta = pred[1:] - pred[:-1]
                delta[delta < 0] = 0.0
                delta[(path_start_idx - 1)[:-1].long()] = 0.0
                return ((pred-cost_ret)**2).mean() + eta*(delta**2).mean()
...
if __name__ == '__main__':
    ...
    ascpo(lambda : create_env(args), actor_critic=core.MLPActorCritic,
        ...
        k=args.k, omega_1=args.omega1, omega_2=args.omega2, train_vc_iters=args.train_vc_iters, resume=args.resume)