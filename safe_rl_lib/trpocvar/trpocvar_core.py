...
class MLPActorCritic(nn.Module):
    ...
    def __init__(self, observation_space, action_space, 
        ...
        # build augmented cost value function
        self.vc_aug  = MLPCritic(obs_dim, hidden_sizes, activation).to(self.device)

        # initialize value-at-risk
        self.var = 0.0
    ...
    def step(self, obs):
        with torch.no_grad():
            ...
            v = self.v(obs)
            vc = self.vc(obs)
            vc_aug = self.vc_aug(obs)
        return a.cpu().numpy(), v.cpu().numpy(), vc.cpu().numpy(), vc_aug.cpu().numpy(), \
            logp_a.cpu().numpy(), pi.mean.cpu().numpy(), torch.log(pi.stddev).cpu().numpy()
