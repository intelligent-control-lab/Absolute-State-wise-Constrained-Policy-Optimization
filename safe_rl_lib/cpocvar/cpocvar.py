...
class CPOCVaRBuffer:
    """
    A buffer for storing trajectories experienced by a CPOCVaR agent interacting
    ...
    def finish_path(self, last_val=0, last_cost_val=0, last_aug_cost_val=0, ep_cost=0):
        """
...
def cpocvar(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0,
            steps_per_epoch=4000, epochs=50, gamma=0.99, alpha=0.3, vf_lr=1e-3, vcf_lr=1e-3,
            vc_aug_lr=1e-3, var_lr=1e-2, train_v_iters=80, train_vc_iters=80, train_vc_aug_iters=80,
            lam=0.97, max_ep_len=1000, target_kl=0.01, target_cost=1.5, logger_kwargs=dict(),
            save_freq=20, backtrack_coeff=0.8, backtrack_iters=100, model_save=False,
            cost_reduction=0, exp_name=None, resume=None):
    """
    Constrained Policy Optimization - Conditional Value-at-Risk
    ...
    buf = CPOCVaRBuffer(obs_dim, act_dim, local_steps_per_epoch, gamma, lam)
    ...
    def compute_cost_pi(data, cur_pi):
        ...
        """
        obs, act, aug_cost_adv, logp_old = data['obs'], data['act'], data['aug_cost_adv'], data['logp']
        
        # Surrogate cost function 
        _, logp = cur_pi(obs, act)
    ...
    def compute_loss_pi(data, cur_pi):
        """
        The reward objective for CPOCVaR (CPOCVaR policy loss)
        """
    ...
    def update():
        ...
        # CPOCVaR policy update core impelmentation 
        ...
        if b.T @ b <= 1e-8 and c < 0:
            ...
            # cost grad is nonzero: CPOCVaR update!
    ...
    for epoch in range(epochs):
        for t in range(local_steps_per_epoch):
            ...
            # save and log
            # running cost in augmented CMDP is zero
            ...
            if terminal or epoch_ended:
                ...
                buf.finish_path(v, vc, vc_aug, ep_cost)
                if terminal:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--target_kl', type=float, default=0.02) # the kl divergence limit for CPOCVaR
    ...
    parser.add_argument('--exp_name', type=str, default='cpocvar')
    parser.add_argument('--model_save', action='store_false', default=True)
    ...
    cpocvar(lambda : create_env(args), actor_critic=core.MLPActorCritic,
            ...
            cost_reduction=args.cost_reduction, exp_name=exp_name, resume=args.resume)
