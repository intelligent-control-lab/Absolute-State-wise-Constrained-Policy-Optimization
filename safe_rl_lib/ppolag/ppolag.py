...
class PPOLAGBuffer:
    """
    A buffer for storing trajectories experienced by a PPOLAG agent interacting
...
def ppolag(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
    ...
    buf = PPOLAGBuffer(obs_dim, act_dim, local_steps_per_epoch, gamma, lam)

    # Set up function for computing PPOLAG policy loss
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', type=str, default='ppolag')
    ...
    ppolag(lambda : create_env(args), actor_critic=core.MLPActorCritic, save_freq=args.save_freq,
        ac_kwargs=dict(hidden_sizes=[args.hid]*args.l), gamma=args.gamma, 
        seed=args.seed, steps_per_epoch=args.steps, epochs=args.epochs,
        logger_kwargs=logger_kwargs, target_kl=args.target_kl, model_save=model_save,
        target_cost=args.target_cost, lam_ratio=args.lam_ratio, lam_lr=args.lam_lr, resume=args.resume)